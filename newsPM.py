...
def get_naver_news(keyword):
    ...
    for article in articles:
        ...
        if "시간 전" in date_text or "분 전" in date_text or "TOP" in date_text:
            ...
            if article_time > now - timedelta(hours=15):
                if is_keyword_in_article(link, '티웨이'):
...
def MarinaNews(keywords, get_naver_news, send_email):
    # 각 키워드에 대해 가져온 기사들을 담는 리스트
    articles = [article for keyword in keywords for article in get_naver_news(keyword)]
    print(articles)
    # 기사의 제목과 링크를 담는 세트를 초기화하여 중복을 제거
    unique_articles = set()
    for article in articles:
        # 기사의 (제목, 링크) 튜플을 세트에 추가
        unique_articles.add((article[1], article[2]))
    ...
    <style>
      ...
      a { text-decoration: none; color: blue; font-size: 14px;line-height: 2;}
    ...
    # 중복이 제거된 기사들을 순회하면서 HTML에 추가
    for title, link in unique_articles:
        article_link = f'{article_counter}. <a href="{link}">{title}</a>'
        article_links.append(article_link)
        article_counter += 1

    # 기사 링크들을 HTML에 추가
    html += '<br>'.join(article_links)
    html += "</body></html>"
...
def send_email(html_content):
    ...
    # 복수의 수신자 이메일 주소들
    # receiver_emails = ["sanghunseo@koreanair.com","jhoon_kim@koreanair.com","skyukim@koreanair.com","hwijunjang@koreanair.com",]  # 수신자 이메일 주소 리스트
    receiver_emails = ["hwijunjang@koreanair.com", "kimjihwan@koreanair.com"]

    # 이메일 메시지 설정
    msg = MIMEMultipart('alternative')
    msg['From'] = sender_email
    msg['To'] = ", ".join(receiver_emails)  # 수신자 이메일 주소들을 콤마로 구분하여 설정
    msg['Subject'] = f"Project Maria 관련 최신 뉴스-{current_time}"
